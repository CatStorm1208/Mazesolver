...
public class Main {
    ...
    public static void main(String[] args) throws IOException, InvalidConnectionException {
        Parser.parse(args[0]);
        returnStack.push(new NumeratedNode((byte) -16));
        loop();
    }
        ...
        try {
            while (true) {
                ...
                if (top.getNumerator() >= top.getMaxNumerator() && !returnStack.isEmpty()) returnStack.pop();
                    ...
                    returnStack.pop();
                } else if (StoredNodes.INSTANCE.getAllNodes().size() > returnStack.size()
                        && top.peekNext().id != -16 && !checkReturnStack(top.peekNext().id)
                        && top.getNumerator() <= top.getMaxNumerator()) {
                    returnStack.push(new NumeratedNode(top.next().id));
